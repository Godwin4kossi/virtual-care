...
export default function DoctorsPage({ onBook }: DoctorsPageProps) {
  ...
  return (
    <div style={{ maxWidth: 980, margin: "0 auto", padding: "36px clamp(16px,4vw,40px)" }}>

      {/* Header */}
      <div style={{ marginBottom: 32 }}>
        <SectionLabel>FIND CARE</SectionLabel>
        <h1 style={{ fontSize: 32, fontWeight: 900, color: T.navy, letterSpacing: -1, marginBottom: 8 }}>
          Our Specialists
        </h1>
        <p style={{ color: T.slate, fontSize: 15 }}>
          {DOCTORS.length} doctors available across {SPECIALTIES.length - 1} specialties
        </p>
      </div>
      ...
      <div style={{
        ...
        border: `1.5px solid ${T.borderLight}`, padding: 20, marginBottom: 28,
      }}>
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
          {/* Search */}
          <div style={{ position: "relative", flex: 1, minWidth: 200 }}>
            <span style={{ position:"absolute",left:12,top:"50%",transform:"translateY(-50%)",fontSize:15,color:T.slateLight }}>
              🔍
            </span>
            <input
              value={search}
              ...
              style={{
                ...
                fontSize: 14, color: T.navy, background: T.bg,
          ...
          <label style={{
            ...
            fontSize: 13, fontWeight: 600, color: T.slate, cursor: "pointer",
            ...
            <span style={{
              ...
              {availOnly && <span style={{ color: T.white, fontSize: 11, fontWeight: 900 }}>✓</span>}
            </span>
            Available Today
          </label>
        </div>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 14 }}>
          {SPECIALTIES.map(s => (
            <Pill key={s} label={s} active={specialty === s} onClick={() => setSpecialty(s)} />
          ))}
        </div>
      </div>

      {/* Results count */}
      <div style={{ fontSize: 13, color: T.slateLight, marginBottom: 20, fontWeight: 500 }}>
